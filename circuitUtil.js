...
let Bot = function(client, config, menu) {
    ...
    this.logEvent = function(evt) {
        ...
        logger.debug(`[MONAS]:`, JSON.stringify(evt));
    };
    ...
    this.sayHi = async function() {
        ...
        client.addTextItem(conv.convId, self.buildConversationItem(null, `Hi from ${user.displayName}`,
            `I am ready. Use "@${user.displayName} help" to see available commands`));
    };
    ...
    */
    ...
    this.dial = async function(convId, itemId, params) {
        ...
        try {
            ...
            currentCall = await client.dialNumber(params[0], null, {audio: false, video: false});
        ...
        try {
            ...
            if (!call) {
                if (conv.type !== Circuit.Enums.ConversationType.DIRECT) {
                    call = await client.startConference(conv.convId, {audio: false, video: false});
                    botState.setState(states.STARTCONF);
                } else {
                    conv.participants.forEach(async function(participant) {
                        if (participant !== user.userId) {
                            call = await client.makeCall(participant, {audio: false, video: false});
                            botState.setState(states.STARTCONF);
                        }
                    });
                }
            } else if (conv.type === Circuit.Enums.ConversationType.DIRECT) {
                client.answerCall(call.callId, {audio: false, video: false});
